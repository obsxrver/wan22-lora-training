...
class TrainRequest(BaseModel):
    ...
    noise_mode: Literal["both", "high", "low", "dual"] = "both"
...
class TrainParams(BaseModel):
    ...
    dual: Dict[str, Any] = Field(default_factory=dict)
...
def _write_train_params(payload: TrainRequest, dataset_config_path: Path) -> Optional[str]:
    ...
    data = {
        ...
        "dual": payload.train_params.dual or {},
    }
    shared = data["shared"]
...
async def start_training(payload: TrainRequest) -> Dict:
    ...
    if noise_mode == "dual":
        try:
            if LOW_LOG.exists() or LOW_LOG.is_symlink():
                LOW_LOG.unlink()
            LOW_LOG.symlink_to(HIGH_LOG)
        except OSError:
            pass
