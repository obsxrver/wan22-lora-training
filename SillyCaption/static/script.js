(() => {
  ...
  const config = window.SILLYCAPTION_CONFIG || {};
  ...
  const ui = {
    ...
    btnSaveLibrary: el('btnSaveLibrary'),
  };
...
✓ "zoomed in and centered the subject"`,
  ...
  async function refreshItemsFromFiles() {
    ...
    await displayDatasetFromFiles(inputFiles, { clearOnEmpty: true });
  }

  async function displayDatasetFromFiles(inputFiles, { clearOnEmpty = true } = {}) {
  ...
  async function buildItemsFromFiles(inputFiles) {
    ...
    for (const file of mediaInputs) {
      ...
      if (isImage(file)) {
        ...
        items.push({ kind: 'image', name: displayName, type: file.type, dataUrl, file });
      } else if (isVideo(file)) {
        items.push({ kind: 'video', name: displayName, type: file.type, dataUrls: null, file });
      }
    }
  ...
  async function startCaptioning({ onlyUncaptioned = false } = {}) {
    ...
    const inputFiles = Array.from(ui.files?.files || []);
    const hasUploadedItems = inputFiles.length > 0;
    const hasCurrentItems = Array.isArray(state.currentItems) && state.currentItems.length > 0;

    if (!hasUploadedItems && !hasCurrentItems) {
  ...
  function guessMimeType(filename) {
    ...
    if (lower.endsWith('.txt') || lower.endsWith('.caption')) return 'text/plain';
    return 'application/octet-stream';
  }

  async function loadDatasetArchive(blob) {
    if (typeof JSZip === 'undefined') {
      alert('ZIP library not available.');
      return;
    }
    try {
      ...
      zip.forEach((relativePath, entry) => {
        ...
        tasks.push(
          entry.async('blob').then((fileBlob) => {
            const type = guessMimeType(relativePath);
            const file = new File([fileBlob], relativePath, { type });
      ...
      await displayDatasetFromFiles(files, { clearOnEmpty: true });
      ui.files && (ui.files.value = '');
    } catch (error) {
      console.error('Failed to load dataset archive:', error);
      alert(`Failed to load dataset: ${error?.message || error}`);
    }
  }

  async function buildDatasetArchive() {
    if (typeof JSZip === 'undefined') {
    ...
    for (const item of state.currentItems) {
      ...
      const captionText = (getCardText(item.card) || '').trim();
      if (captionText) {
        zip.file(`${baseName}.txt`, captionText);
      }
      ...
      if (item.kind === 'image') {
        ...
        if (blob) {
          const imageName = item.file?.name || item.name || `${baseName}.png`;
  ...
  async function importActiveDataset() {
    if (!apiBase) {
      alert('Dataset import is unavailable: API base missing.');
      return;
    }
    ui.progressText.textContent = 'Importing dataset...';
    ui.progressText.className = 'processing';
    try {
      const response = await fetch(`${apiBase}/api/import/active`, {
        method: 'GET',
        credentials: 'same-origin',
      });
      if (response.status === 204 || response.status === 404) {
        alert('Training dataset folder is empty.');
        ui.progressText.textContent = 'Idle';
        ui.progressText.className = '';
        return;
      }
      ...
      await loadDatasetArchive(blob);
      ui.progressText.textContent = 'Dataset imported';
      ui.progressText.className = 'success';
    } catch (error) {
      console.error('Import failed:', error);
      ui.progressText.textContent = `Import failed: ${error?.message || error}`;
      ui.progressText.className = 'error';
    }
  }
  ...
  async function exportActiveDataset() {
    if (!apiBase) {
      alert('Dataset export is unavailable: API base missing.');
      return;
    }
    try {
      const blob = await buildDatasetArchive();
      const formData = new FormData();
      formData.append('archive', blob, 'dataset.zip');
      ui.progressText.textContent = 'Exporting dataset...';
      ui.progressText.className = 'processing';
      const response = await fetch(`${apiBase}/api/export/active`, {
        method: 'POST',
        body: formData,
        credentials: 'same-origin',
      });
      ...
      ui.progressText.textContent = 'Dataset exported to training folder';
      ui.progressText.className = 'success';
    } catch (error) {
      console.error('Export failed:', error);
      ui.progressText.textContent = `Export failed: ${error?.message || error}`;
      ui.progressText.className = 'error';
    }
  }

  async function refreshLibraryOptions() {
    if (!apiBase || !ui.librarySelect) return;
    try {
      const response = await fetch(`${apiBase}/api/library/list`, {
        method: 'GET',
        credentials: 'same-origin',
      });
      if (!response.ok) {
      ...
      if (datasets.length === 0) {
        ...
        ui.btnLoadLibrary && (ui.btnLoadLibrary.disabled = true);
        return;
      }
      ...
      if (ui.btnLoadLibrary) ui.btnLoadLibrary.disabled = false;
    } catch (error) {
      console.error('Failed to load dataset library:', error);
      if (ui.btnLoadLibrary) ui.btnLoadLibrary.disabled = true;
    }
  }

  async function loadLibraryDataset() {
    if (!apiBase) {
      alert('Dataset library is unavailable.');
      return;
    }
    const name = ui.librarySelect?.value;
    if (!name) {
      alert('Select a dataset to load.');
      return;
    }
    ui.progressText.textContent = `Loading "${name}"...`;
    ui.progressText.className = 'processing';
    try {
      const response = await fetch(`${apiBase}/api/library/load/${encodeURIComponent(name)}`, {
        method: 'GET',
        credentials: 'same-origin',
      });
      if (!response.ok) {
      ...
      const blob = await response.blob();
      await loadDatasetArchive(blob);
      ui.progressText.textContent = `Loaded dataset "${name}"`;
      ui.progressText.className = 'success';
    } catch (error) {
      console.error('Failed to load library dataset:', error);
      ui.progressText.textContent = `Load failed: ${error?.message || error}`;
      ui.progressText.className = 'error';
    }
  }

  async function saveLibraryDataset() {
    if (!apiBase) {
      alert('Dataset library is unavailable.');
      return;
    }
    const name = ui.libraryName?.value.trim();
    if (!name) {
      alert('Enter a dataset name to save.');
      return;
    }
    try {
      const blob = await buildDatasetArchive();
      const formData = new FormData();
      formData.append('archive', blob, `${name}.zip`);
      formData.append('name', name);
      ui.progressText.textContent = `Saving "${name}"...`;
      ui.progressText.className = 'processing';
      const response = await fetch(`${apiBase}/api/library/save`, {
        method: 'POST',
        body: formData,
        credentials: 'same-origin',
      });
      ...
      ui.progressText.textContent = `Saved dataset "${name}"`;
      ui.progressText.className = 'success';
      await refreshLibraryOptions();
    } catch (error) {
      console.error('Failed to save dataset:', error);
      ui.progressText.textContent = `Save failed: ${error?.message || error}`;
      ui.progressText.className = 'error';
    }
  }
